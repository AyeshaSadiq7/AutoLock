...
public class Collections {
		...
		static class CheckedEntrySet<K, V> implements Set<Entry<K, V>> {
			...
			public Object[] toArray() {
				...
				for (int i = 0; i < source.length; i++)
					dest[i] = new CheckedEntry<K, V>((Entry<K, V>) source[i], valueType);
				s_valueTypeLock.writeLock().unlock();
				return dest;
			}
			@Perm(requires = "share(s) * immutable(valueType) in alive", ensures = "share(s) * immutable(valueType) in alive")
			public <T> T[] toArray(T[] a) {
				...
				for (int i = 0; i < arr.length; i++)
					arr[i] = new CheckedEntry<K, V>((Entry<K, V>) arr[i], valueType);
				s_valueTypeLock.writeLock().unlock();
				if (arr.length > a.length)
					return (T[]) arr;
				System.arraycopy(arr, 0, a, 0, arr.length);
			...
			public boolean contains(Object o) {
				...
				try {
					s_valueTypeLock.writeLock().lock();
					return s.contains(new CheckedEntry<K, V>((Entry<K, V>) o, valueType));
				} finally {
					s_valueTypeLock.writeLock().unlock();
				}
			}
	...
	private static class SingletonMap<K, V> extends AbstractMap<K, V> implements Serializable {
		...
		public Set<Entry<K, V>> entrySet() {
			if (entrySet == null)
				entrySet = Collections.<Entry<K, V>>singleton(new SimpleImmutableEntry<K, V>(k, v));
			return entrySet;
		}
		@Perm(requires = "none(values) * unique(values) in alive", ensures = "none(values) * unique(values) in alive")
		public Collection<V> values() {
	...
	private static class ReverseComparator2<T> implements Comparator<T>, Serializable {
		...
		@Perm(requires = "unique(cmp) * unique(cmp) in alive", ensures = "unique(cmp) * unique(cmp) in alive")
		public int compare(T t1, T t2) {
			try {
				cmpLock.writeLock().lock();
				return cmp.compare(t2, t1);
			} finally {
				cmpLock.writeLock().unlock();
			}
		}
	}
