...
public class SeqIntegral {
	...
	@Perm(requires = "share(area) in alive", ensures = "share(area) in alive")
	...
	public static Double f(final Double x1) {
		return x1 * x1;
	}
	@Perm(requires = "no permission in alive", ensures = "no permission in alive")
	...
	@Perm(requires = "immutable(x1) * immutable(x2) * share(area) in alive", ensures = "immutable(x1) * immutable(x2) * share(area) in alive")
