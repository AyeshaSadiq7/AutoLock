...
public class SeqLUD extends LUD {
	...
	@Perm(requires = "immutable(row) * immutable(col) * immutable(BLOCK_SIZE) in alive", ensures = "immutable(row) * immutable(col) * immutable(BLOCK_SIZE) in alive")
	...
	public void auxLowerSolve(final MatrixPosition posMa, final MatrixPosition posMb, MatrixPosition posL,
	...
	@Perm(requires = "share(isFirstCall) * immutable(row) * immutable(col) * immutable(BLOCK_SIZE) in alive", ensures = "share(isFirstCall) * immutable(row) * immutable(col) * immutable(BLOCK_SIZE) in alive")
	public void calcLU(MatrixPosition pos, int numOfBlocks) {
		try {
			BLOCK_SIZE_col_isFirstCall_rowLock.writeLock().lock();
			if (isFirstCall) {
