...
public class SeqShellSort {
	...
	@Perm(requires = "unique(data) * none(DEFAULT_SIZE) in alive", ensures = "unique(data) * none(DEFAULT_SIZE) in alive")
	SeqShellSort() {
...
class Client {
	...
	@Perm(requires = "share(data) * immutable(gaps) in alive", ensures = "share(data) * immutable(gaps) in alive")
