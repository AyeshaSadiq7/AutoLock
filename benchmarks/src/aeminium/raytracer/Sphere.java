...
public class Sphere extends Primitive implements java.io.Serializable {
	...
	Vec c;
	double r, r2;
	@Perm(requires = "unique(c) * unique(r) * unique(r2) in alive", ensures = "unique(c) * unique(r) * unique(r2) in alive")
	...
	@Perm(requires = "pure(c) * immutable(P) * pure(D) * none(r2) * immutable(t) * full(enter) * full(prim) * pure(surf) * immutable(surf) in alive", ensures = "pure(c) * immutable(P) * pure(D) * none(r2) * immutable(t) * full(enter) * full(prim) * pure(surf) * immutable(surf) in alive")
	...
	@Perm(requires = "pure(c) in alive", ensures = "pure(c) in alive")
	...
	@Perm(requires = "pure(c) * none(r) in alive", ensures = "pure(c) * none(r) in alive")
	...
	@Perm(requires = "full(c) in alive", ensures = "full(c) in alive")
