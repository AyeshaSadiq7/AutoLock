...
public class JGFRayTracerBench extends RayTracer implements JGFSection3 {
	...
	@Perm(requires = "share(width) * share(height) * immutable(datasizes) * pure(size) * share(scene) * share(numobjects) in alive", ensures = "share(width) * share(height) * immutable(datasizes) * pure(size) * share(scene) * share(numobjects) in alive")
	...
	@Perm(requires = "pure(width) * pure(height) * immutable(width) in alive", ensures = "pure(width) * pure(height) * immutable(width) in alive")
	...
	@Perm(requires = "pure(checksum) * pure(size) in alive", ensures = "pure(checksum) * pure(size) in alive")
	...
	@Perm(requires = "unique(scene) * unique(lights) * unique(prim) in alive", ensures = "unique(scene) * unique(lights) * unique(prim) in alive")
	...
	@Perm(requires = "unique(numobjects) * unique(width) * unique(height) * unique(width) in alive", ensures = "unique(numobjects) * unique(width) * unique(height) * unique(width) in alive")
