...
public class RayTracer {
	...
	int size;
	int numobjects;
	@Perm(requires = "immutable(surf) * immutable(shine) * share(ks) * share(kt) in alive", ensures = "immutable(surf) * immutable(shine) * share(ks) * share(kt) in alive")
	Scene createScene() {
		int x = 0;
		int y = 0;
		...
		for (int i = 0; i < nx; i++) {
			for (int j = 0; j < ny; j++) {
				for (int k = 0; k < nz; k++) {
					...
					D_P_brightness_checksum_color_enter_kd_ks_kt_lights_pos_prim_shine_surf_view_x_y_zLock.writeLock()
							.lock();
					p.surf.shine = 15.0;
					p.surf.ks = 1.5 - 1.0;
					p.surf.kt = 1.5 - 1.0;
	...
	@Perm(requires = "unique(lights) * unique(prim) * share(view) in alive", ensures = "unique(lights) * unique(prim) * share(view) in alive")
	...
	@Perm(requires = "pure(view) * share(D) * pure(x) * pure(y) * pure(z) * share(checksum) in alive", ensures = "pure(view) * share(D) * pure(x) * pure(y) * pure(z) * share(checksum) in alive")
	...
	@Perm(requires = "immutable(t) * pure(prim) * pure(prim) * pure(surf) * pure(enter) in alive", ensures = "immutable(t) * pure(prim) * pure(prim) * pure(surf) * pure(enter) in alive")
	...
	@Perm(requires = "immutable(ior) in alive", ensures = "immutable(ior) in alive")
	...
	@Perm(requires = "immutable(t) * pure(prim) * pure(D) * pure(prim) in alive", ensures = "immutable(t) * pure(prim) * pure(D) * pure(prim) in alive")
	...
	@Perm(requires = "share(scene) * pure(width) * pure(height) * immutable(width) in alive", ensures = "share(scene) * pure(width) * pure(height) * immutable(width) in alive")
