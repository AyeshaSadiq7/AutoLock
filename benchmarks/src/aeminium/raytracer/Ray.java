...
final public class Ray {
	...
	public Vec P, D;
	@Perm(requires = "unique(P) * none(x) * none(y) * none(z) * unique(D) in alive", ensures = "unique(P) * none(x) * none(y) * none(z) * unique(D) in alive")
	...
	@Perm(requires = "unique(P) * unique(D) in alive", ensures = "unique(P) * unique(D) in alive")
	public Ray() {
		D_PLock.writeLock().lock();
		P = new Vec();
		D = new Vec();
	...
	@Perm(requires = "immutable(P) * pure(x) * pure(D) * pure(y) * pure(z) in alive", ensures = "immutable(P) * pure(x) * pure(D) * pure(y) * pure(z) in alive")
	...
	@Perm(requires = "immutable(P) * pure(D) in alive", ensures = "immutable(P) * pure(D) in alive")
