...
public class Hashtable<K, V> extends Dictionary<K, V> implements Map<K, V>, Cloneable, Serializable {
	...
	public Set<K> keySet() {
		try {
			count_entrySet_hash_key_keySet_loadFactor_modCount_next_table_threshold_value_valuesLock.writeLock().lock();
			if (keySet == null)
				keySet = hashtablejava167.entity.Collections.synchronizedSet(new KeySet(), this);
			return keySet;
		} finally {
			count_entrySet_hash_key_keySet_loadFactor_modCount_next_table_threshold_value_valuesLock.writeLock()
					.unlock();
	...
	private class Enumerator<T> implements Enumeration<T>, Iterator<T> {
		...
		@Perm(requires = "unique(iterator) * immutable(lastReturned) * share(table) * immutable(hash) * pure(next) * immutable(next) * share(modCount) * unique(expectedModCount) * share(count) * unique(lastReturned) in alive", ensures = "unique(iterator) * immutable(lastReturned) * share(table) * immutable(hash) * pure(next) * immutable(next) * share(modCount) * unique(expectedModCount) * share(count) * unique(lastReturned) in alive")
		public void remove() {
			try {
				count_entry_expectedModCount_hash_index_iterator_key_lastReturned_modCount_next_table_type_valueLock
						.writeLock().lock();
