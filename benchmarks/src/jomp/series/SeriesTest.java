...
class SeriesTest {
	...
	int array_rows;
	double[][] TestArray;
	@Perm(requires = "unique(TestArray) * pure(array_rows) in alive", ensures = "unique(TestArray) * pure(array_rows) in alive")
	void buildTestData() {
		TestArray_array_rowsLock.writeLock().lock();
		TestArray = new double[2][array_rows];
		TestArray_array_rowsLock.writeLock().unlock();
	}
	@Perm(requires = "share(TestArray) * pure(array_rows) in alive", ensures = "share(TestArray) * pure(array_rows) in alive")
	void Do() {
		double omega;
		TestArray_array_rowsLock.writeLock().lock();
	...
	private double TrapezoidIntegrate(double x0, double x1, int nsteps, double omegan, int select) {
		double x;
		double dx;
		double rvalue;
		x = x0;
	...
	@Perm(requires = "no permission in alive", ensures = "no permission in alive")
	private double thefunction(double x, double omegan, int select) {
		switch (select) {
			case 0 :
	...
	@Perm(requires = "unique(TestArray) in alive", ensures = "unique(TestArray) in alive")
	void freeTestData() {
		TestArray_array_rowsLock.writeLock().lock();
		TestArray = null;
		TestArray_array_rowsLock.writeLock().unlock();
