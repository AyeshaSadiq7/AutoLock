...
public class Linpack {
	...
	@Perm(requires = "unique(a_one_dim) in alive", ensures = "unique(a_one_dim) in alive")
	...
	@Perm(requires = "no permission in alive", ensures = "no permission in alive")
	final void daxpy(double dx[], int n, double dy[], double da, int dx_off, int incx, int dy_off, int incy) {
		int i, ix, iy;
		if ((n > 0) && (da != 0)) {
			if (incx != 1 || incy != 1) {
				ix = 0;
	...
	@Perm(requires = "no permission in alive", ensures = "no permission in alive")
	final double ddot(double dx[], double dy[], int n, int dx_off, int incx, int dy_off, int incy) {
		double dtemp;
		int i, ix, iy;
		dtemp = 0;
		if (n > 0) {
	...
	@Perm(requires = "no permission in alive", ensures = "no permission in alive")
	final void dscal(double dx[], int n, double da, int dx_off, int incx) {
		int i, nincx;
		if (n > 0) {
			if (incx != 1) {
				nincx = n * incx;
	...
	@Perm(requires = "no permission in alive", ensures = "no permission in alive")
	final int idamax(double dx[], int n, int dx_off, int incx) {
		double dmax, dtemp;
		int i, ix, itemp = 0;
		if (n < 1) {
			itemp = -1;
