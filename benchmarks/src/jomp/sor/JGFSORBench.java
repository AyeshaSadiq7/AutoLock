...
public class JGFSORBench extends SOR implements JGFSection2 {
	...
	@Perm(requires = "unique(R) in alive", ensures = "unique(R) in alive")
	public void JGFinitialise() {
		G_R_datasizes_lastElement_lastElementcopy_sizeLock.writeLock().lock();
		R = new Random(RANDOM_SEED);
		G_R_datasizes_lastElement_lastElementcopy_sizeLock.writeLock().unlock();
	}
	@Perm(requires = "share(lastElement) * pure(datasizes) * pure(size) * share(lastElementcopy) * share(G) * pure(R) in alive", ensures = "share(lastElement) * pure(datasizes) * pure(size) * share(lastElementcopy) * share(G) * pure(R) in alive")
	...
	@Perm(requires = "pure(Gtotal) * pure(size) in alive", ensures = "pure(Gtotal) * pure(size) in alive")
	...
	@Perm(requires = "unique(G) * unique(datasizes) in alive", ensures = "unique(G) * unique(datasizes) in alive")
