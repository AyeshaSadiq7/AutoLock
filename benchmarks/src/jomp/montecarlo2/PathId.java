...
public class PathId extends Universal {
	...
	@Perm(requires = "none(prompt) * none(DEBUG) * unique(name) in alive", ensures = "none(prompt) * none(DEBUG) * unique(name) in alive")
	...
	@Perm(requires = "share(name) in alive", ensures = "share(name) in alive")
	...
	@Perm(requires = "share(startDate) in alive", ensures = "share(startDate) in alive")
	...
	@Perm(requires = "share(endDate) in alive", ensures = "share(endDate) in alive")
	...
	@Perm(requires = "share(dTime) in alive", ensures = "share(dTime) in alive")
	public void setdTime(double dTime) {
		dTime_endDate_name_startDateLock.writeLock().lock();
		this.dTime = dTime;
		dTime_endDate_name_startDateLock.writeLock().unlock();
	}
	@Perm(requires = "share(name) * share(startDate) * share(endDate) * share(dTime) in alive", ensures = "share(name) * share(startDate) * share(endDate) * share(dTime) in alive")
	...
	@Perm(requires = "pure(name) * pure(startDate) * pure(endDate) * pure(dTime) in alive", ensures = "pure(name) * pure(startDate) * pure(endDate) * pure(dTime) in alive")
