...
public class PriceStock extends Universal {
	...
	@Perm(requires = "unique(mcPath) * none(prompt) * none(DEBUG) * unique(DEBUG) * unique(prompt) in alive", ensures = "unique(mcPath) * none(prompt) * none(DEBUG) * unique(DEBUG) * unique(prompt) in alive")
	public PriceStock() {
		super();
		DEBUG_expectedReturnRate_finalStockPrice_mcPath_pathStartValue_pathValue_prompt_randomSeed_taskHeader_volatility_volatility2Lock
				.writeLock().lock();
	...
	@Perm(requires = "immutable(mcPath) * share(pathStartValue) * pure(pathStartValue) * share(pathStartValue) in alive", ensures = "immutable(mcPath) * share(pathStartValue) * pure(pathStartValue) * share(pathStartValue) in alive")
	...
	@Perm(requires = "share(taskHeader) * share(randomSeed) * pure(randomSeed) in alive", ensures = "share(taskHeader) * share(randomSeed) * pure(randomSeed) in alive")
	...
	public Object getResult() {
		DEBUG_expectedReturnRate_finalStockPrice_mcPath_pathStartValue_pathValue_prompt_randomSeed_taskHeader_volatility_volatility2Lock
				.readLock().lock();
		String resultHeader = "Result of task with Header=" + taskHeader + ": randomSeed=" + randomSeed
				+ ": pathStartValue=" + pathStartValue;
		ToResult res = new ToResult(resultHeader, expectedReturnRate, volatility, volatility2, finalStockPrice,
				pathValue);
		DEBUG_expectedReturnRate_finalStockPrice_mcPath_pathStartValue_pathValue_prompt_randomSeed_taskHeader_volatility_volatility2Lock
				.readLock().unlock();
		return (Object) res;
