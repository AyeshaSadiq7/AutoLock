...
public class RatePath extends PathId {
	...
	@Perm(requires = "none(prompt) * none(DEBUG) in alive", ensures = "none(prompt) * none(DEBUG) in alive")
	...
	@Perm(requires = "unique(prompt) * unique(DEBUG) in alive", ensures = "unique(prompt) * unique(DEBUG) in alive")
	...
	@Perm(requires = "none(prompt) * none(DEBUG) * unique(pathValue) * unique(nAcceptedPathValue) in alive", ensures = "none(prompt) * none(DEBUG) * unique(pathValue) * unique(nAcceptedPathValue) in alive")
	...
	@Perm(requires = "unique(pathValue) * unique(nAcceptedPathValue) * unique(pathDate) in alive", ensures = "unique(pathValue) * unique(nAcceptedPathValue) * unique(pathDate) in alive")
	...
	public void incpathValue(double[] operandPath) throws DemoException {
		try {
			nAcceptedPathValue_pathDate_pathValueLock.writeLock().lock();
	...
	@Perm(requires = "share(pathValue) in alive", ensures = "share(pathValue) in alive")
	public void incpathValue(double scale) throws DemoException {
		nAcceptedPathValue_pathDate_pathValueLock.writeLock().lock();
		if (pathValue == null)
			for (int i = 0; i < pathValue.length; i++)
				pathValue[i] *= scale;
	...
	@Perm(requires = "pure(pathDate) in alive", ensures = "pure(pathDate) in alive")
	public int[] getpathDate() throws DemoException {
		try {
			nAcceptedPathValue_pathDate_pathValueLock.readLock().lock();
	...
	@Perm(requires = "share(pathDate) in alive", ensures = "share(pathDate) in alive")
	...
	@Perm(requires = "pure(pathValue) * pure(nAcceptedPathValue) * immutable(COMPOUNDED) in alive", ensures = "pure(pathValue) * pure(nAcceptedPathValue) * immutable(COMPOUNDED) in alive")
	...
	@Perm(requires = "pure(pathValue) * pure(nAcceptedPathValue) * immutable(NONCOMPOUNDED) in alive", ensures = "pure(pathValue) * pure(nAcceptedPathValue) * immutable(NONCOMPOUNDED) in alive")
	...
	@Perm(requires = "unique(pathValue) * unique(pathDate) * full(nAcceptedPathValue) in alive", ensures = "unique(pathValue) * unique(pathDate) * full(nAcceptedPathValue) in alive")
