...
public class IDEATest {
	...
	int[] Z;
	int[] DK;
	@Perm(requires = "pure(plain1) * pure(crypt1) * pure(Z) * pure(plain2) * pure(DK) in alive", ensures = "pure(plain1) * pure(crypt1) * pure(Z) * pure(plain2) * pure(DK) in alive")
	public void Do() {
	...
	@Perm(requires = "unique(plain1) * pure(array_rows) * unique(crypt1) * unique(plain2) * unique(userkey) * unique(Z) * unique(DK) in alive", ensures = "unique(plain1) * pure(array_rows) * unique(crypt1) * unique(plain2) * unique(userkey) * unique(Z) * unique(DK) in alive")
	void buildTestData() {
	...
	@Perm(requires = "share(Z) * pure(userkey) in alive", ensures = "share(Z) * pure(userkey) in alive")
	...
	@Perm(requires = "share(Z) * share(DK) in alive", ensures = "share(Z) * share(DK) in alive")
	...
	@Perm(requires = "unique(plain1) * unique(crypt1) * unique(plain2) * unique(userkey) * unique(Z) * unique(DK) in alive", ensures = "unique(plain1) * unique(crypt1) * unique(plain2) * unique(userkey) * unique(Z) * unique(DK) in alive")
	void freeTestData() {
		DK_Z_array_rows_crypt1_plain1_plain2_userkeyLock.writeLock().lock();
		plain1 = null;
		crypt1 = null;
