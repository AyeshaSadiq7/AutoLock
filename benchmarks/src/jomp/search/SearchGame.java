...
public class SearchGame extends TransGame {
	...
	@Perm(requires = "share(nodes) * share(msecs) * pure(plycnt) * pure(height) * immutable(colthr) * pure(columns) * pure(posed) in alive", ensures = "share(nodes) * share(msecs) * pure(plycnt) * pure(height) * immutable(colthr) * pure(columns) * pure(posed) in alive")
	int solve() {
		int i, side;
		int x, work, score;
		long poscnt;
	...
	@Perm(requires = "share(nodes) * share(plycnt) * share(height) * immutable(colthr) * share(columns) * share(posed) * unique(history) in alive", ensures = "share(nodes) * share(plycnt) * share(height) * immutable(colthr) * share(columns) * share(posed) * unique(history) in alive")
	int ab(int alpha, int beta) {
		...
		try {
			...
			if (besti > 0) {
				for (i = 0; i < besti; i++)
					history[side][height[av[i]] << 3 | av[i]]--;
