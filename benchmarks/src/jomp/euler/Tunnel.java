...
public class Tunnel {
	...
	public void initialise() throws Exception {
		...
		try {
				...
				for (i = 0; i < imax; i++) {
					for (j = 0; j < jmax; j++) {
						...
						newval[k][i][j] = (1.0 - xf) * (1.0 - yf) * oldval[k][iold][jold]
								...
								+ xf * (1.0 - yf) * oldval[k][iold + 1][jold] + xf * yf * oldval[k][iold + 1][jold + 1];
					}
				}
			}
			...
			for (i = 1; i < imax; i++)
				for (j = 1; j < jmax; ++j)
					aofTunnel[i][j] = 0.5 * ((xnode[i][j] - xnode[i - 1][j - 1]) * (ynode[i - 1][j] - ynode[i][j - 1])
							- (ynode[i][j] - ynode[i - 1][j - 1]) * (xnode[i - 1][j] - xnode[i][j - 1]));
			oldval = null;
			newval = null;
		} finally {
			a_aofTunnel_b_c_cff_d_deltat_error_f_g_ihat_imax_imaxin_jhat_jmax_jmaxin_jminusff_jplusff_newval_oldval_opg_pff_pg_pg1_r_rhoff_scale_seta_size_sxi_tff_tg_tg1_uff_ug_ug1_vff_xnode_ynodeLock
	...
	@Perm(requires = "pure(imax) * pure(jmax) * pure(b) * pure(c) * pure(d) * pure(a) in alive", ensures = "pure(imax) * pure(jmax) * pure(b) * pure(c) * pure(d) * pure(a) in alive")
	private void calculateStateVar(double localpg[][], double localtg[][], Statevector localug[][]) {
		...
		for (int i = 1; i < imax; i++) {
			for (j = 1; j < jmax; ++j) {
				temp = localug[i][j].b;
				temp2 = localug[i][j].c;
				localtg[i][j] = localug[i][j].d / localug[i][j].a
						- 0.5 * (temp * temp + temp2 * temp2) / (localug[i][j].a * localug[i][j].a);
	...
	@Perm(requires = "pure(imax) * pure(jmax) * share(a) * pure(r) * share(b) * share(c) * share(d) * pure(ynode) * pure(xnode) * pure(f) * pure(g) in alive", ensures = "pure(imax) * pure(jmax) * share(a) * pure(r) * share(b) * share(c) * share(d) * pure(ynode) * pure(xnode) * pure(f) * pure(g) in alive")
	...
	@Perm(requires = "pure(imax) * pure(jmax) * share(c) * share(a) * pure(g) * share(b) * share(d) in alive", ensures = "pure(imax) * pure(jmax) * share(c) * share(a) * pure(g) * share(b) * share(d) in alive")
	...
	@Perm(requires = "pure(imax) * pure(jmax) * share(b) * share(a) * pure(f) * share(c) * share(d) in alive", ensures = "pure(imax) * pure(jmax) * share(b) * share(a) * pure(f) * share(c) * share(d) in alive")
	...
	@Perm(requires = "pure(imax) * pure(jmax) * pure(xnode) * pure(ynode) * pure(b) * pure(ug) * pure(c) * pure(tg) * share(deltat) * pure(aofTunnel) * pure(a) * none(ntime) in alive", ensures = "pure(imax) * pure(jmax) * pure(xnode) * pure(ynode) * pure(b) * pure(ug) * pure(c) * pure(tg) * share(deltat) * pure(aofTunnel) * pure(a) * none(ntime) in alive")
		...
		for (int i = 1; i < imax; i++)
			for (j = 1; j < jmax; ++j) {
				...
				deltat[i][j] = safety_factor * 2.8284 * aofTunnel[i][j]
						/ ((Math.abs(q) + Math.abs(r)) / ug[i][j].a + c * Math.sqrt(xxi * xxi + yxi * yxi + xeta * xeta
								+ yeta * yeta + 2.0 * Math.abs(xeta * xxi + yeta * yxi)));
			}
		a_aofTunnel_b_c_cff_d_deltat_error_f_g_ihat_imax_imaxin_jhat_jmax_jmaxin_jminusff_jplusff_newval_oldval_opg_pff_pg_pg1_r_rhoff_scale_seta_size_sxi_tff_tg_tg1_uff_ug_ug1_vff_xnode_ynodeLock
				.writeLock().unlock();
		a_aofTunnel_b_c_cff_d_deltat_error_f_g_ihat_imax_imaxin_jhat_jmax_jmaxin_jminusff_jplusff_newval_oldval_opg_pff_pg_pg1_r_rhoff_scale_seta_size_sxi_tff_tg_tg1_uff_ug_ug1_vff_xnode_ynodeLock
	...
	@Perm(requires = "none(iter) in alive", ensures = "none(iter) in alive")
...
class Statevector {
	...
	double c;
	double d;
	@Perm(requires = "unique(a) * unique(b) * unique(c) * unique(d) in alive", ensures = "unique(a) * unique(b) * unique(c) * unique(d) in alive")
	Statevector() {
	...
	@Perm(requires = "share(a) * share(b) * share(c) * share(d) in alive", ensures = "share(a) * share(b) * share(c) * share(d) in alive")
...
class Vector2 {
	...
	double ihat;
	double jhat;
	@Perm(requires = "unique(ihat) * unique(jhat) in alive", ensures = "unique(ihat) * unique(jhat) in alive")
	Vector2() {
	...
	@Perm(requires = "pure(ihat) * pure(jhat) in alive", ensures = "pure(ihat) * pure(jhat) in alive")
